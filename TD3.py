...
class Agent():
    ...
    def train(self, replay_buffer:ReplayBuffer):
        ...
        self.writer.add_scalar('CriticLoss/steps',critic_loss, self.total_it)
        ...
        if(self.total_it % self.policy_freq == 0):
            ...
            self.writer.add_scalar('ActorLoss/steps',actor_loss, self.total_it)
    ...
    def load(self, filename):
        ...
        self.critic_optimizer.load_state_dict(torch.load("models/checkpoint/" + filename + "_critic_optimizer", map_location='cpu'))#optional
        self.critic_target = copy.deepcopy(self.critic)

        self.actor.load_state_dict(torch.load("models/checkpoint/" + filename + "_actor", map_location='cpu'))
        self.actor_optimizer.load_state_dict(torch.load("models/checkpoint/" + filename + "_actor_optimizer", map_location='cpu'))#optional
        self.actor_target = copy.deepcopy(self.actor)
    
    def freeze(self):
        for a in self.actor.parameters():
            a.requires_grad = False
        for a in self.actor.mu.parameters():
            a.requires_grad = True

            
        for a in self.actor_target.parameters():
            a.requires_grad = False
        for a in self.actor_target.mu.parameters():
            a.requires_grad = True


        for a in self.critic.parameters():
            a.requires_grad = False
        for a in self.critic.l3.parameters():
            a.requires_grad = True
        for a in self.critic.l6.parameters():
            a.requires_grad = True
            
        for a in self.critic_target.parameters():
            a.requires_grad = False
        for a in self.critic_target.l3.parameters():
            a.requires_grad = True
        for a in self.critic_target.l6.parameters():
            a.requires_grad = True